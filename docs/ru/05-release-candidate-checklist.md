...
| --- | --- | --- | --- |
| Local quality | `scripts/quality.py` проходит внутри Podman development image | `dist/release-candidate/runs/<RUN_ID>/release-gate.txt` и локальный вывод команды | `PASS` только если прогон зеленый на финальном состоянии workspace |
...
| Release candidate run | `scripts/release_candidate.py` пересобирает финальный локальный evidence pack | `dist/release-candidate/runs/<RUN_ID>/summary.md` и `latest.txt` | `PASS` только если прогон сделан по финальному состоянию workspace |
