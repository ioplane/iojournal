/*
 * iojournal -- io_uring file sink benchmark harness
 *
...
static ij_event_t ij_make_event(void)
{
    ...
    ij_event_t event = {
        ...
        .level = IJ_LEVEL_INFO,
        .event_name = "bench.file_sink_uring",
        .message = "benchmark event payload for explicit io_uring append-only file sink scenario",
        .logger = "bench.file_sink_uring",
        ...
        .source_file = "bench/bench_file_sink_uring.c",
        .source_line = 1U,
...
static int ij_make_temp_path(char *buffer, size_t buffer_size)
{
    ...
    written = snprintf(buffer, buffer_size, "/tmp/iojournal-bench-file-uring-%ld-%ld.ndjson",
                           ...
                           uintmax_t file_bytes)
{
    ...
    if (mode == OUTPUT_TSV) {
        ...
        printf("bench_file_sink_uring\tappend_file_io_uring\t%zu\t%" PRIu64 "\t%.2f\n", iterations,
               elapsed_ns, ns_per_op);
        printf("meta\tstatus_sink\t%" PRIu64 "\n", ij_status_sink);
        return 0;
    }

    puts("iojournal benchmark: file sink io_uring");
    printf("iterations: %zu\n", iterations);
    printf("file-bytes: %ju\n", file_bytes);
    printf("append_file_io_uring iterations=%-8zu elapsed=%12" PRIu64 " ns ns/op=%10.2f\n",
...
int main(int argc, char **argv)
{
    ...
    config = (ij_logger_config_t){
        ...
        .logger_name = "bench.file_sink_uring",
        .file_path = path,
    ...
    if (logger->file_sink.active_backend != IJ_FILE_BACKEND_IO_URING) {
