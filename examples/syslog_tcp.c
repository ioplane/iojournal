...
int main(void)
{
    ...
    ij_event_t event = {
        ...
        .event_name = "example.syslog.tcp",
        .message = "tcp example payload",
        .logger = "examples.syslog_tcp",
        .attributes = NULL,
        .attribute_count = 0U,
        .trace_id = NULL,
        .span_id = NULL,
        ...
        .source_file = "examples/syslog_tcp.c",
        .source_line = 1U,
    ...
    ij_logger_config_t config = {
        ...
        .logger_name = "examples.syslog_tcp",
        .file_path = NULL,
    ...
    if (setsockopt(listener_fd, SOL_SOCKET, SO_RCVTIMEO, &timeout, sizeof(timeout)) != 0) {
    ...
    bytes_read = recv(peer_fd, buffer, sizeof(buffer) - 1U, 0);
    if (bytes_read <= 0) {
        (void)ij_logger_shutdown(logger);
        (void)close(peer_fd);
        (void)close(listener_fd);
